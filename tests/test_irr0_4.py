...
class IRR04Tests(unittest.TestCase):
    ...
    def test_review_ui_exposes_ladder_controls_without_production_data_import(self) -> None:
        ...
        self.assertIn('useState(IRR04_PILOT_STORIES[0])', page)
        self.assertIn('useState<"conditions" | "span_review" | "semantic_ladder">("semantic_ladder")', page)
        self.assertIn("IRR0.4 语义递进重读实验", page)
        self.assertIn("IRR0.4 Semantic Ladder", page)
        self.assertIn("IRR0.3 Span Review", page)
        self.assertIn("Baseline Conditions", page)
        self.assertLess(page.index("IRR0.4 Semantic Ladder"), page.index("IRR0.3 Span Review"))
        self.assertLess(page.index("IRR0.3 Span Review"), page.index("Baseline Conditions"))
