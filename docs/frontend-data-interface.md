...
## UX2 Person and Story index

UX2 adds a deterministic index over the same reader-ready SC1 projection. The
compact files are generated by `scripts/build_ux2_index.py`:

```text
data/derived/ux2-person-index.json
data/derived/ux2-story-index.json
site/public/generated/ux2/person-index.json
site/public/generated/ux2/story-index.json
```

They contain only the exposed Person IDs/names/surname groups and the exposed
Story IDs/category labels/category-local numbers. They do not add candidates,
unresolved identities, annotation-only entities, or historical research
records to the initial SC1 bundle. The `/index` view fetches these two small
static projections only when opened; its Person and Story buttons call the
existing exploration state and panels rather than implementing new detail
views. Story references are displayed as `门类 · 编号` while internal Story IDs
remain in the projection for navigation and validation.
