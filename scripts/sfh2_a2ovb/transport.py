"""Bounded DeepSeek transport for A2OVB without raw-response persistence."""
...
class BoundaryClient:
    def __init__(self, *, live: bool, tool: Mapping[str, Any]) -> None:
        self.live = live
        self.tool = tool
        self.attempts = 0

    def probe(self, messages: list[Mapping[str, Any]]) -> tuple[Mapping[str, Any] | None, dict[str, Any]]:
        from smoke_deepseek import call_deepseek

        payload = {"task": "contract_probe", "case_id": "schema-probe"}
    ...
    def call(self, *, case_id: str, system: str, payload: Mapping[str, Any]) -> tuple[Mapping[str, Any] | None, dict[str, Any]]:
        from smoke_deepseek import call_deepseek

        row = _base("boundary_validator", case_id, payload)
        attempt_history: list[dict[str, Any]] = []
        for attempt in (1, 2):
            if self.attempts >= MAX_PROVIDER_ATTEMPTS:
                row.update({"attempt": attempt, "classification": "provider_attempt_budget_exhausted", "valid": False, "usage": {}})
                return None, row
            self.attempts += 1
            started = time.monotonic()
            try:
...
def summarize(rows: list[Mapping[str, Any]], *, live: bool, provider_attempts: int, boundary_count: int) -> dict[str, Any]:
    ...
    return {
        "schema": "sfh2-a2ovb-provider-accounting-v1",
        "model": MODEL,
        "prompt_version": PROMPT_VERSION,
        "endpoint": STRICT_ENDPOINT,
        "live": live,
        ...
        "boundary_validator_calls": sum(row.get("stage") == "boundary_validator" for row in rows),
        "non_boundary_calls": 0,
        "expected_boundary_validator_calls": boundary_count,
