"""Bounded, cacheable strict-function transport for the pilot."""
...
from .common import MODEL, OUT, ROOT, PILOT_VERSION, PROMPT_VERSIONS, STRICT_ENDPOINT, canonical_json, read_json, stable_hash, text, write_json


def _usage(response: Mapping[str, Any]) -> dict[str, int]:
    row = response.get("usage") if isinstance(response.get("usage"), Mapping) else {}
    return {"prompt_tokens": int(row.get("prompt_tokens") or 0), "completion_tokens": int(row.get("completion_tokens") or 0), "total_tokens": int(row.get("total_tokens") or 0)}
...
def summarize_transport_records(records: list[Mapping[str, Any]], *, live: bool) -> dict[str, Any]:
    """Summarize one immutable transport record stream.

    Keeping this separate from ``PilotClient`` lets a deterministic replay
    report both its cache activity and the cost of the original bounded live
    run without making another provider call.
    """
...
class PilotClient:
    """Strict transport with two attempts maximum and immutable raw responses."""
    ...
    def call(self, *, stage: str, unit_id: str, system: str, payload: Mapping[str, Any], tool: Mapping[str, Any], function_name: str, max_tokens: int) -> Mapping[str, Any] | None:
        ...
        packet_hash = stable_hash({"pilot_version": PILOT_VERSION, "stage": stage, "prompt_version": prompt_version, "model": MODEL, "system": system, "payload": payload, "tool": tool})
        ...
        for attempt in (1, 2):
            ...
            row: dict[str, Any] = {"sequence": sequence, "attempt": attempt, "stage": stage, "unit_id": unit_id, "packet_hash": packet_hash, "prompt_version": prompt_version, "model": MODEL, "start_time": _now()}
            try:
