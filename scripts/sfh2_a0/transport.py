"""Strict, bounded, cacheable transport for the isolated A0 pilot."""
...
from .common import (
    FUNCTION_NAMES, MAX_PROVIDER_ATTEMPTS, MODEL, OUT, PILOT_VERSION,
    PROMPT_VERSIONS, ROOT, STRICT_ENDPOINT, canonical_json, read_json,
...
def summarize_transport_records(rows: list[Mapping[str, Any]], *, live: bool) -> dict[str, Any]:
    ...
    non_cache = {"cache_hit", "offline_cache_miss"}
    return {
        "schema": "sfh2-a0-transport-v1",
        "model": MODEL,
        ...
        "new_live_attempts": sum(row.get("classification") not in non_cache for row in rows) if live else 0,
...
class PilotClient:
    """Use cached raw responses first and enforce the total attempt budget."""
    ...
    def __init__(self, run_dir: Path, *, live: bool) -> None:
        ...
        self.live_attempts = sum(row.get("classification") not in {"cache_hit", "offline_cache_miss"} for row in self.records)
    ...
    def call(self, *, stage: str, unit_id: str, system: str, payload: Mapping[str, Any], tool: Mapping[str, Any], max_tokens: int) -> Mapping[str, Any] | None:
        ...
        packet_hash = stable_hash({
            "pilot_version": PILOT_VERSION, "stage": stage,
            "prompt_version": PROMPT_VERSIONS[stage], "model": MODEL,
            "system": system, "payload": payload, "tool": tool,
        })
        ...
        for attempt in (1, 2):
            with self.lock:
                if self.live_attempts >= MAX_PROVIDER_ATTEMPTS:
                    raise RuntimeError("sfh2_a0_provider_attempt_budget_exceeded")
            ...
            row: dict[str, Any] = {
                ...
                "packet_hash": packet_hash, "prompt_version": PROMPT_VERSIONS[stage],
                "model": MODEL, "temperature": 0, "thinking": {"type": "disabled"}, "start_time": _now(),
            }
            try:
                ...
                self.cache[packet_hash] = {
                    "raw_path": str(raw_path.relative_to(ROOT)), "stage": stage,
                    "unit_id": unit_id, "prompt_version": PROMPT_VERSIONS[stage], "model": MODEL,
                }
                ...
                row.update({
                    ...
                    "elapsed_seconds": round(time.monotonic() - started, 3), "end_time": _now(),
                })
                self._record(row)
        return None
