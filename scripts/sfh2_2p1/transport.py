"""Strict, cacheable provider transport for SFH2.2-P1."""
...
class PilotClient:
    """Two-attempt strict transport with an isolated P1 cache and raw run."""
    ...
    def call(self, *, stage: str, unit_id: str, system: str, payload: Mapping[str, Any], tool: Mapping[str, Any], function_name: str, max_tokens: int) -> Mapping[str, Any] | None:
            ...
            if path.is_file():
                ...
                if extracted is not None and error is None:
                    self._record({
                        ...
                        "classification": "cache_hit", "raw_path": str(path.relative_to(ROOT)),
        ...
        if not self.live:
            self._record({
                "stage": stage, "unit_id": unit_id, "packet_hash": packet_hash,
                "classification": "offline_cache_miss", "usage": {}, "elapsed_seconds": 0,
            })
            return None
        for attempt in (1, 2):
            with self.lock:
            ...
            try:
                ...
                self.cache[packet_hash] = {
                    ...
                    "unit_id": unit_id, "prompt_version": prompt_version, "model": MODEL,
                }
