"""Bounded A2OR provider transport with raw responses kept outside Git."""
...
from .contracts import occurrence_function_tool
...
def error_details(exc: BaseException) -> dict[str, Any]:
    ...
    if decoded is not None:
        safe_body = json.dumps(_scrub(decoded), ensure_ascii=False, sort_keys=True, separators=(",", ":"))
    else:
        safe_body = body
    result: dict[str, Any] = {
...
class A2ORClient:
    ...
    def probe(self, tool: Mapping[str, Any], messages: list[Mapping[str, Any]]) -> dict[str, Any]:
        from smoke_deepseek import call_deepseek

        row = _row_base("schema_probe", "schema-probe", {"probe": True})
        started = time.monotonic()
        if not self.live:
            row.update({"attempt": 0, "classification": "offline_no_provider_call", "valid": True, "usage": {}, "elapsed_seconds": 0})
            return row
        self.attempts += 1
        try:
            ...
            payload, parse_error = _extract(response)
            row.update({
                "attempt": 1,
                ...
                "response_witness_sha256": hashlib.sha256(json.dumps(response, ensure_ascii=False, sort_keys=True).encode("utf-8")).hexdigest(),
            })
        except Exception as exc:  # transport detail is bounded/sanitized below
            row.update({"attempt": 1, "classification": "provider_request_failure", **error_details(exc), "retryable": False, "valid": False, "usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}})
    ...
    def call(self, *, case_id: str, system: str, payload: Mapping[str, Any], tool: Mapping[str, Any]) -> tuple[Mapping[str, Any] | None, dict[str, Any]]:
        ...
        row = _row_base("occurrence_function", case_id, payload)
        if not self.live:
            row.update({"attempt": 0, "classification": "offline_no_provider_call", "valid": False, "usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}, "elapsed_seconds": 0})
            return None, row
        attempt_history: list[dict[str, Any]] = []
        for attempt in (1, 2):
            if self.attempts >= MAX_PROVIDER_ATTEMPTS:
                row.update({"attempt": attempt, "classification": "provider_attempt_budget_exhausted", "valid": False, "usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}, "elapsed_seconds": 0})
                return None, row
            self.attempts += 1
            started = time.monotonic()
            try:
                ...
                if extracted is not None and parse_error is None:
                    if attempt_history:
                ...
                row.update({"attempt": attempt, "classification": "provider_request_failure", **details, "retryable": retryable, "valid": False, "usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}, "elapsed_seconds": round(time.monotonic() - started, 3)})
                if not retryable:
